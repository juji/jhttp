...
class jhttp {
		...
		return Q(0).then(()=>{
			...
			req.on('timeout',(e)=>{
				...
				p.reject({
					...
					headers: 0,
					body: '',
					text: 'Request timeout' + (reqopt.timeout ? ` ${reqopt.timeout}ms` : '')
				});
			});

			if(reqopt.timeout)
				req.setTimeout(reqopt.timeout)
	...
	__requestObj(opt){
		...
		// set proxy
		if(opt.proxy){
			var proxy = urlUtils.parse(opt.proxy);
			var urlDest = urlUtils.parse(opt.url);
			req.hostname = proxy.hostname;
			req.port = proxy.port;
			req.path = opt.url;
			req.headers['Host'] = urlDest.hostname;
			req.protocol = proxy.protocol;
			if(!req.port) req.port = proxy.protocol == 'https:' ? 443 : 80;
		}
	...
	// request interface
	request(obj, isredirect, streamer ){

		var opt = JSON.parse(JSON.stringify(obj))
		var stream = streamer;
